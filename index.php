...
$total_pengajuan = mysqli_fetch_assoc(db_query($conn, "SELECT COUNT(*) as total FROM surat_pengajuan", ""))['total'];
$total_berita = mysqli_fetch_assoc(db_query($conn, "SELECT COUNT(*) as total FROM berita WHERE diterbitkan = 1", ""))['total'];
$berita_terbaru = db_query($conn, "SELECT * FROM berita WHERE diterbitkan = 1 ORDER BY created_at DESC LIMIT 3", "");
?>

<div class="hero-section text-white text-center py-5 mb-5 rounded-3">
    <h1 class="display-4">Selamat Datang di <?= e(DESA_NAMA) ?></h1>
    <p class="lead"><?= e(DESA_KECAMATAN) ?>, <?= e(DESA_KABUPATEN) ?>, <?= e(DESA_PROVINSI) ?></p>
    ...
    <div class="col-md-12">
        <div class="card">
            <div class="card-body">
                ...
                <p class="card-text">Assalamu'alaikum Warahmatullahi Wabarakatuh. Selamat datang di website resmi <?= e(DESA_NAMA) ?>. Kami berkomitmen untuk memberikan pelayanan terbaik kepada seluruh warga desa. Melalui website ini, kami harapkan informasi dapat tersampaikan dengan cepat dan transparan. Terima kasih atas partisipasi aktif seluruh warga dalam membangun desa kita tercinta.</p>
                <p class="text-end fw-bold"><?= e(KEPALA_DESA) ?></p>
            </div>
        </div>
    </div>
</div>
...
<div class="row">
    ...
    <div class="col-md-4 mb-4">
        <div class="card h-100">
            <div class="card-body">
                <h5 class="card-title"><?= e($berita['judul']) ?></h5>
                <p class="text-muted"><i class="bi bi-calendar"></i> <?= date('d M Y', strtotime($berita['created_at'])) ?> | <i class="bi bi-person"></i> <?= e($berita['penulis']) ?></p>
                <p class="card-text"><?= e(substr($berita['isi'], 0, 150)) ?>...</p>
                <a href="berita_detail.php?id=<?= (int)$berita['id'] ?>" class="btn btn-success btn-sm">Baca Selengkapnya</a>
            </div>
        </div>
    </div>
    <?php endwhile; ?>
